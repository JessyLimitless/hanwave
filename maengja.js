/* =========================================================================
 * 유진이의 한자교실 — 맹자(孟子) 핵심 명문·일화 35장 게임 엔진
 *  데이터: window.MAENGJA_DATA (maengja-data.js) — 7편 구절 객체 배열
 *  루프: 인의(仁義)의 길(35 디딤돌) → 장 클릭 → 낭독 → 빈칸 → 짝맞추기 → 풀이 → 물음 → 도장
 * ======================================================================= */
(function () {
  'use strict';
  const $ = (s, r) => (r || document).querySelector(s);
  const DATA = Array.isArray(window.MAENGJA_DATA) ? window.MAENGJA_DATA.slice().sort((a, b) => a.ch - b.ch) : [];
  ...
  const READ = (function () {
    ...
    const extra = window.MAENGJA_READ_EXTRA || {};
  ...
  let muted = localStorage.getItem('maengja_muted') === '1';
  function ctx() {
    if (actx) return actx;
  ...
  applyMute();
  if (sBtn) sBtn.addEventListener('click', () => { muted = !muted; localStorage.setItem('maengja_muted', muted ? '1' : '0'); applyMute(); if (!muted) { ctx(); sfx.drop(); } });
  let kicked = false;
  ...
  function renderMeter() {
    const total = DATA.length || 35, done = progress.cleared.length;
    const f = $('#dao-fill'); if (f) f.style.width = (total ? (done / total * 100) : 0) + '%';
    const l = $('#dao-lab'); if (l) l.innerHTML = '<span>인의(仁義)의 길</span><span><b>' + done + '</b> / ' + total + ' 장</span>';
    ...
    if (rb) {
      ...
      else { rb.textContent = '✓ 35장 모두 통과'; rb.disabled = true; }
    }
    const cb = $('#complete-banner'); if (cb) cb.classList.toggle('hidden', done < total);
  }
  function renderPath() {
    const host = $('#stones'); if (!host) return;
    if (!DATA.length) { host.innerHTML = '<p style="grid-column:1/-1; text-align:center; color:var(--text-soft); padding:40px">맹자 데이터를 불러오지 못했어요. (maengja-data.js)</p>'; return; }
    host.innerHTML = '';
    const SECTION_LBL = {
      '양혜왕': '梁惠王 <small>양혜왕 · 어진 정치</small>',
      '공손추': '公孫丑 <small>공손추 · 호연지기와 사단</small>',
      '등문공': '滕文公 <small>등문공 · 바른 도리</small>',
      '이루': '離婁 <small>이루 · 나를 돌아보기</small>',
      '만장': '萬章 <small>만장 · 사람과 글</small>',
      '고자': '告子 <small>고자 · 본성과 의로움</small>',
      '진심': '盡心 <small>진심 · 마음을 다하여</small>'
    };
  ...
  function openChapter(c) {
    ...
    function frame(inner) {
      popup.innerHTML =
        '<div class="ph-top"><span class="ch-tag">孟子 · ' + (c.src ? c.src + ' · ' : '') + '제 ' + c.ch + ' 장</span>' +
    ...
    function renderRecite() {
      ...
      const full = c.full || (window.MAENGJA_FULL || {})[c.ch];
    ...
    function renderReward() {
      ...
      frame(
        ...
        '<h3>제 ' + c.ch + '장 도장 획득!</h3>' +
        '<p class="r-title serif">' + c.title + '</p>' +
        '<p class="r-read">' + readOf(c.title) + '</p>' +
        '<p class="r-ko">' + (c.titleKo || '') + '</p>' +
        '<p style="margin-top:10px; color:var(--teal); font-weight:800">인의(仁義)의 길 ' + progress.cleared.length + ' / ' + (DATA.length || 35) + ' 장</p>' +
  ...
  if (reset) reset.addEventListener('click', () => {
    if (confirm('맹자 진행(도장)을 모두 초기화할까요?')) { progress = { cleared: [] }; save(); renderPath(); toast('초기화했어요.'); }
  });
